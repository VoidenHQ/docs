...
agent-callable capability — see the [field reference](../core-features-section/voiden-blocks/mcp-tool-block-reference.md)
for every field on the block itself, its Parameters table, and its Verification table. Publishing
that as a real, running MCP server is a separate concern from letting an agent editor simply run
requests in a project — they're two different surfaces, each with exactly one job:
...
| **[`voiden agent`](./voiden-cli.md#voiden-agent--register-with-an-agent-editor)** | Bundled into the Voiden app's own `voiden` command (nothing extra to install if you already have the app) — registers *this project* with an agent editor, writing `.mcp.json` (and the Codex equivalent) pointing at a small, built-in 4-tool server (`list_void_files`, `list_requests`, `run_request`, `write_result`), **not** at `@voiden/mcp`. `voiden agent --remove` undoes registration. CI-only environments without the app use `voiden-runner mcp install` instead — same registration, same 4 tools, standalone. |
...
requests in this project" registration never touches it.
...
Each row in the tool's **Parameters** table then answers two separate questions:

| Field | Question it answers |
|---|---|
| **`source`** | *Who provides the value?* `agent` — supplied fresh on every call, part of the tool's schema. `environment` — resolved silently from the project's env files (or the serving process's own environment when publishing without a project — see [Hosting](#hosting-a-published-server) below); never shown to or settable by the agent. |
| **`binds`** | *Where does the value go?* The exact `{{token}}` name in the request it fills in. |

Both fields are always required, regardless of source — `source` alone can't tell Voiden
*where* to put a value, because a request can have several fields and the request body is opaque
text to Voiden, not a schema it matches param names against. `binds` is the address; `source` is
where the postal service picks the letter up from.
...
- name: Run MCP server
  env:
    ...
    # any source: environment params your tools need
...
publishing involved) is a different, much lighter command — [`voiden agent`](./voiden-cli.md#voiden-agent--register-with-an-agent-editor)
(bundled in the app) or `voiden-runner mcp install` (standalone, for CI/no-app environments).
...
- **A cross-file `requestFilePath`/verify `filePath` still holding an absolute path from wherever
  ...
  [path portability](../core-features-section/voiden-blocks/mcp-tool-block-reference.md#a-note-on-path-portability). Concretely: open
...
[Verification table reference](../core-features-section/voiden-blocks/mcp-tool-block-reference.md#verification-table-toolverifies-rows)
...
**Why do I need a `{{token}}` if I already set `source: agent`?**
`source` says *who* provides the value; it doesn't say *where* it goes. A request can have
several fields — the token is the only thing that tells Voiden which one a given param controls.
...
**What happens if a `source: environment` param has no value anywhere when I publish?**
`publish` runs a pre-flight check before serving anything: if that param's `binds` key isn't in
`--env`/process env and the project has no `.voiden/env-*.yaml` at all, that tool is excluded
from the served set (with a clear reason printed) rather than silently sent with a literal
unresolved `{{token}}` in it — or add `--strict` to abort the whole publish instead of just
excluding that one tool. This is best-effort, not an exhaustive check: a multi-environment
project where a specific tool's param doesn't resolve in the *particular* environment it targets,
while some `.voiden/env-*.yaml` file exists, isn't caught yet.
