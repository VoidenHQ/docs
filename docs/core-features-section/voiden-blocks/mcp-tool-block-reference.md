...
Every field on a [`/tool` block](./tool.md), its **Parameters** table, and its **Verification**
table — what each one does, what values it accepts, and what happens if you leave it out. For the
concepts behind `source`/`binds` and how to actually publish what you build here, see
[Publishing Tools as an MCP Server](../../developer-tools/mcp-tool-publish-guide.md).
...
Each row lets an agent (or the environment) control one spot in the request. A row does nothing
unless the request actually contains the `{{token}}` it names in `binds` — see
["How a param actually resolves"](../../developer-tools/mcp-tool-publish-guide.md#how-a-param-actually-resolves) for
the full explanation of why `source` and `binds` are both required and answer different questions.
...
| `required` | boolean | — | `false` | Whether the agent must supply this argument on every call. |
| `description` | string | — | empty | Shown to the agent as this argument's description — what it should pass here, and why. |
| `source` | enum | ✓ | — | `agent` (value supplied fresh on every tool call, part of the schema) or `environment` (resolved silently from the project's `.voiden/env-*.yaml` files, or the serving process's own env when there's no project env file — never shown to or settable by the agent). A `source: environment` param that can't resolve anywhere excludes the tool from being served (or aborts the whole publish under `--strict`) rather than sending a request with a literal unresolved `{{token}}` in it. |
...
Verification reads real signal from the request it runs: any [assertions](./assertion-block.md)
